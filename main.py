...
for i in range(0,len(nodes)):
    for j in range(0, len(nodes)):
        if nodes[i]["레시피"] == nodes[j]["레시피"]:
            continue
        node1 = nodes[i]
        size1 = nodes[i]["주재료"] + nodes[i]["주재료"] + nodes[i]["양념"]
        node2 = nodes[j]
        size2 = nodes[j]["주재료"] + nodes[j]["주재료"] + nodes[j]["양념"]

        weight = 0

        if len(node1["주재료"] | node2["주재료"]) != 0:
            ratio = len(node1["주재료"] & node2["주재료"]) / len(node1["주재료"] | node2["주재료"])
            weight += math.exp(ratio) - 1
...
#for idx, links in enumerate(graph):
#    print(nodes[idx]["레시피"], end=" ")
#    for element in links:
#        print("(", nodes[element[0]]["레시피"], element[1], "), ", end=" ")
#    print()
print(edge_cnt)
